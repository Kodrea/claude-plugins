...
If the topic is empty or too vague (fewer than 3 words), use AskUserQuestion to ask: "What topic should I research overnight?"
...
```
I'll set up the overnight research pipeline for: "{topic}"

Before I begin, a few quick options:

1. **Rounds** — max research iterations (default: 3)
2. **Domain** — `web`, `codebase`, or `mixed` (default: infer from topic)
3. **Slug** — output directory name (default: auto-generated from topic)

Reply with any overrides (e.g. "rounds 2, domain web") or just "go" to use defaults.
```

Parse the user's reply for:
- **rounds**: integer (default: 3)
- **domain**: `codebase`|`web`|`mixed` (default: infer from topic — URLs suggest web, file paths suggest codebase, ambiguous suggests mixed)
- **slug**: string (default: derive from topic — lowercase, hyphens, max 40 chars)

If the user replies with just "go", "defaults", "ok", or similar affirmative, use all defaults.

---

## Resume from Swarm

If the user specifies `--from swarm` or `--from round-2`, or if the slug's metadata.json contains `"pipeline": "swarm+overnight"`:

1. Read `research/{slug}/metadata.json` and verify `rounds_completed >= 1` and swarm outputs exist.
2. Read `research/{slug}/round-1/compilation.md` (generated by swarm bridge phase).
3. Parse the compilation's Decision section:
   - If FINALIZE: skip to Phase 7 (Adversarial Audit) — the swarm already produced sufficient coverage.
   - If CONTINUE: proceed to Phase 4 (Round 2 Scouting) using the compilation's scout prompts.
4. Set `round_number = rounds_completed + 1` and continue the standard overnight loop from there.
5. The swarm's scout JSONs in `round-1/scout/` and DRAFT.md in `round-1/analysis/` are treated identically to overnight round-1 output.

Skip Phases 0-3 when resuming from swarm — setup and round-1 are already complete.
