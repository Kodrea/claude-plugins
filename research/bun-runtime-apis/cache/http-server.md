...
Bun also supports an alternate syntax.
...
Instead of passing the server options into `Bun.serve`, `export default` it. When Bun sees a file with a `default` export containing a `fetch` handler, it passes it into `Bun.serve` under the hood.
